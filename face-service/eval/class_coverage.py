...
def tile_rects(w, h, cols, rows, overlap):
    """Overlapping tiles covering a frame. Overlap keeps a face that straddles a
    seam from being clipped in both tiles and missed in both."""
...
def merge_detections(items, thresh=0.35):
    """NMS over (bbox, score, payload) tuples. The same face found in two
    overlapping tiles must not be counted, or embedded, twice."""
    kept = []
...
def detect_faces(engine, img, tiles=None, overlap=0.15):
    """Faces in one frame. `tiles` as (cols, rows) runs detection per tile and
    merges, which recovers small faces a single whole-frame pass misses; None runs
    the ordinary single pass. Returns [(bbox_xyxy, det_score, face_obj)]."""
    h, w = img.shape[:2]
    if not tiles:
        return [(tuple(float(v) for v in f.bbox), float(getattr(f, "det_score", 0.0)), f)
